	...
	private String doTest(String text) {
		...
		for (CoreMap sentence : sentences) {
			...
			for (CoreLabel token : tokens) {
				...
				Integer corefClustId = token.get(
						CorefCoreAnnotations.CorefClusterIdAnnotation.class);
				log.debug(
						token.word() + " --> corefClusterID = " + corefClustId);

				CorefChain chain = corefs.get(corefClustId);
				log.debug("matched chain = " + chain);

				if (chain == null
						|| chain.getMentionsInTextualOrder().size() == 1) {
					resolved.add(token.word());
				} else {

					int sentINdx = chain.getRepresentativeMention().sentNum - 1;
					CoreMap corefSentence = sentences.get(sentINdx);
					List<CoreLabel> corefSentenceTokens = corefSentence
					...
					for (int i = reprMent.startIndex; i < reprMent.endIndex; i++) {
						...
						if (reprMent.sentNum != token.sentIndex() + 1) {
							...
							CoreLabel matchedLabel = corefSentenceTokens
									.get(i - 1); //resolved.add(tokens.get(i).word());
